...
const { itemAlreadyExists } = require("../helpers/utils");
...
const sendEmail = async (data, callback) => {
  console.log("listo pa enviar el correo prro");
  const auth = {
    auth: {
...
module.exports = {
  ...
  async sendRegistrationEmailMessage(user) {
    const subject = "Verificar tu Email en Fixtec";
  ...
  async sendResetPasswordEmailMessage(locale, user) {
    ...
    //   user.verification
    // );
    const htmlMessage = "que fue loco olvidaste la contraseña";
    prepareToSendEmail(user, subject, htmlMessage);
  },
};
