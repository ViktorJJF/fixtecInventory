  ...
  <div>
    ...
    >
      ...
      <template v-slot:item.action="{ item }">
        ...
        >Eliminar</v-btn>
        <v-btn v-show="selectButton" color="info" @click="$emit('add-product',item)">Agregar</v-btn>
      </template>
      <template v-slot:item.stock="{ item }">
        {{item.stock}}
        <v-icon color="green" v-show="item.stock>=item.minStock">mdi-checkbox-marked-circle</v-icon>
        <v-tooltip bottom>
          <template v-slot:activator="{ on }">
            <v-icon v-on="on" color="error" v-show="item.stock<item.minStock">mdi-alert-circle</v-icon>
          </template>
      ...
      <template v-slot:item.brandId="{ item }">{{item.brandId?item.brandId.name:"Sin marca"}}</template>
      <template v-slot:item.typeId="{ item }">{{item.typeId?item.typeId.name:"Sin tipo"}}</template>
      <template v-slot:item.colorId="{ item }">{{item.colorId?item.colorId.name:"Sin color"}}</template>
